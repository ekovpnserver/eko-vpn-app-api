...
describe('Testing Application Insights by Residence State', () => {
  ...
  it('should return status code 200', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/state?state=all&filter=count')
  ...
  it('should return status code 200 for a data request', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/state?state=LAGOS&filter=data')
      .set('Authorization', 'Bearer ' + token)
      .end((err, res) => {
        if (err) {}
        res.should.have.status(200)
