...
describe('Testing Application Insights by Program by Residence State', () => {
  ...
  it('should return status code 200', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/program/state?state=all&filter=count')
  ...
  it('should return status code 200 for a data request', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/program/state?state=LAGOS&filter=data')
      .set('Authorization', 'Bearer ' + token)
      .end((err, res) => {
        if (err) {}
        res.should.have.status(200)
  ...
  it('should return status code 400 for a non filtered request', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/state')
      .set('Authorization', 'Bearer ' + token)
      .end((err, res) => {
        if (err) {}
