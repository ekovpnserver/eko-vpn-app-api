...
describe('Testing Application Insights by Gender', () => {
  ...
  it('should return status code 200', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/gender?gender=all&filter=count')
  ...
  it('should return status code 200 for a data request', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/gender?gender=all&filter=data')
      .set('Authorization', 'Bearer ' + token)
      .end((err, res) => {
        if (err) {}
        res.should.have.status(200)
  ...
  it('should return status code 400 for a non filtered request', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/gender')
      .set('Authorization', 'Bearer ' + token)
      .end((err, res) => {
        if (err) {}
