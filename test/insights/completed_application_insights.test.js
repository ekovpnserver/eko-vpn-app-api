...
describe('Testing Completed Application Insights', () => {
  ...
  it('should return status code 200', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/completed?status=all&filter=count')
  ...
  it('should return status code 200 for a data request', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/completed?status=all&filter=data')
      .set('Authorization', 'Bearer ' + token)
      .end((err, res) => {
        if (err) {}
        res.should.have.status(200)
  ...
  it('should return status code 400 for a non filtered request', (done) => {
    chai
      .request(app)
      .get('/api/v1/insight/application/completed')
      .set('Authorization', 'Bearer ' + token)
      .end((err, res) => {
        if (err) {}
